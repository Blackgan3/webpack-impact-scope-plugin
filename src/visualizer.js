...
function generateHtml(report, outPath) {
  const nodes = [
...
function generateChangedModulesHtml(changedModules, htmlPath) {
  const data = JSON.stringify(changedModules);
...
<body>
  ...
  <script>
    ...
    function renderNode(node, container, depth = 0) {
      ...
      card.className = 'card';
      const displayId = node.id.replace(/^\\/Users\\/didi\\/blackdir\\/webpack-impact-scope-plugin\\/e2e\\/mpx-demo\\//, '');
      card.innerHTML = \`
        ...
        <span class="label" title="\${node.id}">\${displayId}</span>
      \`;
...
module.exports = { printTable, generateHtml, generateChangedModulesHtml };